...
export default function Navbar() {
  ...
  const { username, logout } = useSleeperData(); // ✅ Get username & logout
        ...
        <div className="hidden md:flex gap-4 items-center">
          {navLinks.map((link) =>
            link.submenu ? (
              <div key={link.name} className="relative group">
                <button className="px-4 py-2 rounded-full bg-gray-800 hover:bg-gray-700 text-white text-sm transition flex items-center">
                  {link.name}
                </button>
          ...
          {username && (
            <button
              ...
              className="px-4 py-2 bg-red-600 hover:bg-red-700 rounded-full text-white text-sm font-semibold ml-4"
            >
              Logout
            </button>
          )}
        ...
        <div className="md:hidden bg-gray-900 border-t border-gray-700 p-4 space-y-3">
          ...
          {username && (
            <button
              ...
              className="w-full mt-4 bg-red-600 hover:bg-red-700 px-4 py-2 rounded text-white text-sm font-semibold"
            >
              Logout
            </button>
          )}
