...
useEffect(() => {
  ...
  const checkAvailability = (player) => {
    ...
    setSearchedPlayerName(target.name); // ✅ Track last searched name
    };


  if (!username) {
    // ✅ Show input form if no username
    return (
        ...
        <div className="max-w-2xl mx-auto bg-gray-900 p-6 rounded-xl shadow-lg">
          <SearchBox
            ...
            onSelectAndSearch={(player) => {
                ...
                checkAvailability(player); // ✅ Trigger search immediately
            }}
            />
          ...
          {selectedPlayer && (
            <button
              ...
              className="w-full bg-purple-700 hover:bg-purple-600 text-white py-2 rounded mt-4"
            >
              Check Availability
            </button>
          )}
